...
export const AddEventModal = ({ newEvent, onChangeEvent, onAdd, onClose }: AddEventModalProps) => {
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#162b48] dark:bg-gray-900 rounded-xl border border-amber-200/25 dark:border-gray-800/60 shadow-xl w-full max-w-md overflow-hidden animate-scale-in">
        <div className="px-5 py-4 border-b border-amber-100/30 dark:border-gray-800/60 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-sm font-bold text-gray-100 dark:text-gray-100 tracking-tight">Add Event</h2>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-md hover:bg-gray-700/50 dark:hover:bg-gray-700/50 transition-colors">
            <X size={16} className="text-gray-400" />
          </button>
        </div>
        <div className="p-5 space-y-4">
          <div>
            <label className="block text-xs font-medium text-gray-400 dark:text-gray-400 mb-1.5">Title *</label>
            <input
              type="text"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-400 dark:text-gray-400 mb-1.5">Date *</label>
              <input
                type="date"
                value={newEvent.date}
                onChange={(e) => onChangeEvent({ ...newEvent, date: e.target.value })}
                className="w-full px-3 py-2 text-sm bg-[#1c3050] dark:bg-gray-800 border border-gray-700 dark:border-gray-700 rounded-lg text-gray-100 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-gold/30 focus:border-gold"
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-400 dark:text-gray-400 mb-1.5">Time</label>
              <input
                type="time"
                value={newEvent.time}
          ...
          <div>
            <label className="block text-xs font-medium text-gray-400 dark:text-gray-400 mb-1.5">Client Name</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-xs font-medium text-gray-400 dark:text-gray-400 mb-1.5">Description</label>
            <textarea
